from flask import Flask, send_file, render_template, request, redirect, url_for, session, flash, g, abort
...
# ✅ SECURITY: Secret key dari environment variable
...
app.config['MAX_CONTENT_LENGTH'] = int(os.environ.get('MAX_CONTENT_LENGTH', 4194304))
...
# ✅ SECURITY: Database credentials dari environment variable
db_config = {
...
def set_security_headers(response):
    # 🔧 CSP DINONAKTIFKAN untuk development
    # Aktifkan saat production dengan policy yang sudah disesuaikan
    
    # 🛡️ ANTI-CLICKJACKING HEADERS
    response.headers['X-Frame-Options'] = 'DENY'
    response.headers['X-Content-Type-Options'] = 'nosniff'
    response.headers['X-XSS-Protection'] = '1; mode=block'
    
    # 🔒 HSTS - Aktifkan saat sudah HTTPS
    if request.is_secure:
        response.headers['Strict-Transport-Security'] = 'max-age=31536000; includeSubDomains'
    
    # 🔒 CACHE CONTROL
    if request.endpoint and 'static' not in str(request.endpoint) and request.endpoint not in ['download_file', 'robots_txt', 'sitemap_xml']:
        response.headers['Cache-Control'] = 'no-store, no-cache, must-revalidate, private, max-age=0'
        response.headers['Pragma'] = 'no-cache'
        response.headers['Expires'] = '0'
    else:
        if 'static' in str(request.endpoint):
...
# ✅ SECURITY: Safe path join untuk mencegah path traversal
...
def class_required(f):
    ...
    def wrapper(*args, **kwargs):
        if g.user.get('role') == 'sensei' and not session.get('selected_class_id'):
            flash("Silakan pilih kelas terlebih dahulu.", "info")
            return redirect(url_for('dashboard'))
        return f(*args, **kwargs)
    return wrapper
...
# ✅ SECURITY: Download file dengan validasi path
...
def download_file(filename):
    ...
    try:
        ...
        safe_path = safe_join_path(app.config['UPLOAD_FOLDER'], filename)
        
        if not safe_path:
            abort(404)
        
        # ✅ SECURITY: Ensure file is within UPLOAD_FOLDER
        ...
        flash(f"Error membuka file: {str(e)}", "error")
        return redirect(url_for('dashboard'))
...
def admin_edit_user(user_id):
    ...
    if user_id == g.user['id']:
        flash("Anda tidak dapat mengedit data Anda sendiri dari halaman ini. Gunakan menu Profile.", "warning")
        cur.close()
        return redirect(url_for('admin_users'))
    ...
    if request.method == 'POST':
        ...
        db.commit()
        cur2.close()
        flash(f"Data user {username} berhasil diperbarui.", "success")
        cur.close()
        return redirect(url_for('admin_users'))
...
def admin_delete_user(user_id):
    ...
    flash(f"User {user['username']} dan semua data terkait berhasil dihapus.", "success")
    return redirect(url_for('admin_users'))
...
def admin_reset_password(user_id):
    ...
    if request.method == 'POST':
        ...
        if not new_password or not confirm_password:
            flash("Password tidak boleh kosong.", "warning")
...
def create_quiz():
    if g.user.get('role') != 'sensei':
        flash("Akses ditolak. Hanya sensei yang dapat membuat kuis.", "danger")
        return redirect(url_for('dashboard'))
    ...
    if request.method == 'POST':
        ...
        if not title:
            flash("Judul kuis tidak boleh kosong.", "warning")
        ...
        try:
            # Insert kuis baru
            cur.execute("""
                INSERT INTO quizzes (title, class_id, created_by, created_at) 
                VALUES (%s, %s, %s, %s)
            """, (title, class_id, g.user['id'], datetime.now()))
            
            # 🔥 PENTING: Ambil ID kuis yang baru dibuat
            quiz_id = cur.lastrowid
            
            db.commit()
            
            flash(f"✅ Kuis '{title}' berhasil dibuat! Sekarang tambahkan pertanyaan.", "success")
            
            # 🚀 REDIRECT LANGSUNG KE add_question (bukan dashboard)
            return redirect(url_for('add_question', qid=quiz_id))
            
        except Exception as e:
            db.rollback()
            flash(f"❌ Gagal membuat kuis: {str(e)}", "danger")
            return render_template('create_quiz.html', edit=False, quiz=None)
...
def add_question(qid):
    if g.user.get('role') != 'sensei':
        flash("Akses ditolak. Hanya sensei yang dapat menambah pertanyaan.", "danger")
        return redirect(url_for('dashboard'))
    ...
    if request.method == 'POST':
        ...
        if not a or not b or not c or not d or not e:
            flash("Semua opsi jawaban (A-E) harus diisi.", "danger")
            cur.execute("SELECT * FROM quiz_questions WHERE quiz_id=%s ORDER BY id", (qid,))
            questions = cur.fetchall()
            return render_template('add_question.html', quiz=quiz, questions=questions)
        ...
        if correct not in ['a', 'b', 'c', 'd', 'e']:
            flash("Jawaban benar harus salah satu dari A, B, C, D, atau E.", "danger")
            cur.execute("SELECT * FROM quiz_questions WHERE quiz_id=%s ORDER BY id", (qid,))
            questions = cur.fetchall()
            return render_template('add_question.html', quiz=quiz, questions=questions)
        ...
        if file and file.filename:
            ...
            # Generate safe filename
            safe_name = f"quiz_{qid}_q_{datetime.now().strftime('%Y%m%d%H%M%S')}_{secure_filename(file.filename)}"
            filepath = os.path.join(app.config['UPLOAD_FOLDER'], safe_name)
            ...
            try:
                ...
                flash(f'Gagal mengupload gambar: {str(e)}', "danger")
                cur.execute("SELECT * FROM quiz_questions WHERE quiz_id=%s ORDER BY id", (qid,))
                questions = cur.fetchall()
                return render_template('add_question.html', quiz=quiz, questions=questions)
        ...
        try:
            ...
            cur2.execute("""
                ...
                (quiz_id, question, image_path, option_a, option_b, option_c, option_d, option_e, correct_option)
                VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s)
            """, (qid, question, image_path, a, b, c, d, e, correct))
            db.commit()
            flash("✅ Pertanyaan berhasil ditambahkan!", "success")
            
            # REDIRECT ke halaman yang sama agar bisa tambah pertanyaan lagi
            return redirect(url_for('add_question', qid=qid))
            
        except Exception as e:
            db.rollback()
            flash(f"❌ Gagal menyimpan pertanyaan: {str(e)}", "danger")
            # Hapus file gambar jika insert gagal
            if image_path:
                try:
                    os.remove(os.path.join(app.config['UPLOAD_FOLDER'], image_path))
                except:
                    pass
            # Return dengan data questions
            cur.execute("SELECT * FROM quiz_questions WHERE quiz_id=%s ORDER BY id", (qid,))
            questions = cur.fetchall()
            return render_template('add_question.html', quiz=quiz, questions=questions)

    # GET REQUEST - Tampilkan form dan daftar pertanyaan yang sudah ada
    cur.execute("SELECT * FROM quiz_questions WHERE quiz_id=%s ORDER BY id", (qid,))
    questions = cur.fetchall()
...
# ROUTE EDIT QUESTION DENGAN UPLOAD GAMBAR
@app.route('/quiz/<int:qid>/question/<int:qid_q>/edit', methods=['GET', 'POST'])
@login_required
def edit_question(qid, qid_q):
    if g.user.get('role') != 'sensei':
        flash("Akses ditolak. Hanya sensei yang dapat mengedit pertanyaan.", "danger")
        return redirect(url_for('dashboard'))
    ...
    if request.method == 'POST':
        ...
        if file and file.filename:
            ...
            if image_path:
                old_paths = [
                    os.path.join(app.config['UPLOAD_FOLDER'], image_path),
                    os.path.join(app.config['UPLOAD_FOLDER'], image_path.replace('uploads/', '')),
                ...
                for old_file in old_paths:
                    if os.path.exists(old_file):
                        try:
                            os.remove(old_file)
                            print(f"✅ Gambar lama dihapus: {old_file}")
                            break
                        except Exception as e:
        ...
        if request.form.get('remove_image') == 'yes' and image_path:
            ...
            for old_file in old_paths:
                if os.path.exists(old_file):
                    try:
                        os.remove(old_file)
                        print(f"✅ Gambar dihapus: {old_file}")
                        break
                    except Exception as e:
                        print(f"⚠️ Gagal menghapus gambar: {e}")
            image_path = None
        ...
        try:
            ...
            cur2.execute("""
                ...
                SET question=%s, image_path=%s, option_a=%s, option_b=%s, 
                    option_c=%s, option_d=%s, option_e=%s, correct_option=%s
                WHERE id=%s
            """, (qtext, image_path, a, b, c, d, e, correct, qid_q))
            db.commit()
            flash("✅ Pertanyaan berhasil diperbarui!", "success")
            return redirect(url_for('add_question', qid=qid))
        except Exception as e:
            db.rollback()
            flash(f"❌ Gagal memperbarui pertanyaan: {str(e)}", "danger")
            return render_template('edit_question.html', question=question, quiz_id=qid)


# ROUTE DELETE QUESTION DENGAN HAPUS GAMBAR
@app.route('/quiz/<int:qid>/question/<int:qid_q>/delete')
@login_required
def delete_question(qid, qid_q):
    if g.user.get('role') != 'sensei':
        flash("Akses ditolak. Hanya sensei yang dapat menghapus pertanyaan.", "danger")
        return redirect(url_for('dashboard'))
    
    db = get_db()
    cur = db.cursor(dictionary=True)
    
    # Ambil data pertanyaan untuk mendapatkan image_path
    cur.execute("SELECT image_path FROM quiz_questions WHERE id=%s AND quiz_id=%s", (qid_q, qid))
    question = cur.fetchone()
    ...
    if question.get('image_path'):
        ...
        for path in possible_paths:
            if os.path.exists(path):
                try:
                    os.remove(path)
                    print(f"✅ Gambar berhasil dihapus: {path}")
                    break
                except Exception as e:
                    print(f"⚠️ Gagal menghapus gambar: {e}")
    ...
    try:
        cur2 = db.cursor()
        # Hapus jawaban siswa terkait pertanyaan ini
        cur2.execute("DELETE FROM quiz_answers WHERE question_id=%s", (qid_q,))
        # Hapus pertanyaan
        cur2.execute("DELETE FROM quiz_questions WHERE id=%s AND quiz_id=%s", (qid_q, qid))
        db.commit()
        
        flash("✅ Pertanyaan berhasil dihapus!", "success")
    except Exception as e:
        db.rollback()
        flash(f"❌ Gagal menghapus pertanyaan: {str(e)}", "danger")
...
def edit_reply(reply_id):
    ...
    if request.method == 'POST':
        ...
        if not body:
            ...
            return render_template('edit_reply.html', reply=reply, quiz_id=qid)
...
Sitemap: https://lpkyamaguchi-pwt.com/sitemap.xml
"""
...
def sitemap_xml():
    ...
    # ⚠️ GANTI dengan domain production Anda
